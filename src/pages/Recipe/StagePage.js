        ...
        <div className='bg-white rounded shadow p-4 space-y-6'>
          {
            stages.map((s, i) => (
              <StageInput
                key={i}
                deleteDisabled={stages.length === 1}
                setStageValue={setStageValue(i)}
                setStartTime={setTime('startTime', i)}
                setEndTime={setTime('endTime', i)}
          ...
          <ServeTime serveTime={serveTime} setServeTime={setServeTime} />

        </div>
      </div>
      <div className='flex-none bg-white rounded shadow p-4'>
        <button onClick={addStage} type="button" className="w-full px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-green-500 hover:bg-green-400 focus:outline-none focus:border-green-600 focus:shadow-outline-green active:bg-green-600 transition ease-in-out duration-150">
          add stage
        </button>
