  ...
  return isAuthenticated ? (
    <>
      <div className="flex items-center">
        ...
        <div className="ml-3">
          <div className="text-base tracking-wider font-medium leading-6 text-gray-800">{barista.displayName}</div>
          <div className="text-sm font-medium leading-5 text-gray-500">{barista.email}</div>
        </div>
      </div>
      <div className="mt-3 flex flex-col justify-start space-y-1" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
        {links.map(({ to, text }) => <Link key={to} to={to} className="py-1 px-3 rounded-md text-left text-base font-medium text-gray-500 focus:outline-none focus:text-gray-800 focus:bg-gray-100" role="menuitem">{text}</Link>)}
        <button onClick={() => console.log('signout')} className="py-1 px-3 rounded-md text-left text-base font-medium text-gray-500 focus:outline-none focus:text-gray-800 focus:bg-gray-100" role="menuitem">log out</button>
      </div>
    </>
  ) : (
      <>
